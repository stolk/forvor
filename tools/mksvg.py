...
def output( verts, edges, lneqs, pnts, s ) :

	edges = clip_edges( verts, edges, lneqs, pnts )

	edgemap = create_edge_map( verts, edges )
	sys.stderr.write( "edgemap size %d vert list size %d\n" % ( len(edgemap), len(verts) ) )
	...
	output_polygons( polygons, verts, s )
	output_points( pnts, s )
...
if __name__ == "__main__" :
	...
	scl = 20.0
	output( verts, edges, lneqs, pnts, scl )
