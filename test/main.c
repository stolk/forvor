...
main(int argc, char *argv[])
    ...
    voronoi(next) ;
    geomexit(); // free the memory allocated in memory.c
    free(sites); // free the memory allocated when reading the sites intput data.
    sites=0;
    return (0) ;
    }
...
readsites(void)
    ...
    nsites = 0 ;
    sites = (Site *) malloc(4000 * sizeof(Site));
